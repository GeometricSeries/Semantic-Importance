clear all;
clc;
color={[5,80,91]./256,[48,151,164]./256,[255,80,80]./256,[255,170,50]./256,[0.6902,0.4784,0.6314],[200,100,50]./256};
...
y1=[];
for i=1:l_length-40
    y1(i)=(i+40)*U/R+t_ens+t_des;
end
plot(41:l_length,y1,'-','LineWidth',3,'Color',[62,43,109]./256,'HandleVisibility','off')
hold on;
...
plot(1:41,y1,'--','LineWidth',2,'Color',[62,43,109]./256,'HandleVisibility','on')
% hold on;
...
axis([-inf,inf,0,inf])
legend('Conventional Mode','Semantic Mode (Error Free)','Semantic Mode ({\it\gamma}=0)','Semantic Mode ({\it\gamma}=0.25)','Semantic Mode ({\it\gamma}=0.5)','Semantic Mode ({\it\gamma}=0.75)','Semantic Mode ({\it\gamma}=1)','location','Southeast','FontName','Times New Roman','FontSize',12,'LineWidth',1.5);
